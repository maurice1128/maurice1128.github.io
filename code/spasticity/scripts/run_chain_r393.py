...
STAGE = os.path.join(ROOT, "chain_r393")
...
SP_RUNGS = [("050", 0.050), ("070", 0.070), ("100", 0.100), ("150", 0.150)]
WK_RUNGS = [("080", 0.80), ("060", 0.60), ("040", 0.40), ("030", 0.30)]
SP_ROOT = "R151S"
WK_ROOT = "R151W"
    ...
    for k in range(1, len(SP_RUNGS)):
        ...
        for s in SEEDS:
            out.append({
                ...
                "prefix": "R393SPg%s_s%d" % (tag, s),
                "parent_prefix": ("%s_s%d" % (SP_ROOT, s) if k == 1
                                  else "R393SPg%s_s%d" % (ptag, s)),
                "parent_is_root": k == 1})
    for k in range(1, len(WK_RUNGS)):
        ...
        for s in SEEDS:
            out.append({
                ...
                "prefix": "R393WK%s_s%d" % (tag, s),
                "parent_prefix": ("%s_s%d" % (WK_ROOT, s) if k == 1
                                  else "R393WK%s_s%d" % (ptag, s)),
                "parent_is_root": k == 1})
    return out
...
def stage():
    ...
    p = os.path.join(PAPER, "CHAIN_STAGE_r393.json")
...
# ------------------------------------------------------------------ running -----------------
def cell_json(i):
    return os.path.join(PAPER, "RUN_CHAIN_r393_cell%02d.json" % i)
...
def aggregate():
    ...
    out["survival"] = surv
    p = os.path.join(PAPER, "CHAIN_RESULT_r393.json")
...
def rest():
    ...
    for i, c in enumerate(cs):
        ...
        if r.get("status") == "YIELDED":
            write_terminal(len(glob.glob(os.path.join(PAPER, "RUN_CHAIN_r393_cell*.json"))),
...
def headline():
    ...
    p = os.path.join(PAPER, "CHAIN_HEADLINE_r393.json")
    wopen(p).write(json.dumps(out, indent=1) + "\n")
