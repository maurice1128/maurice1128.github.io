"""run_bandfill_r287.py -- launcher for PREREG_bandfill_r287.md (580ec823...).
...
STAGE = os.path.join(ROOT, "bandfill_r287")
...
def cells():
    out = []
    for seed in range(107, 152):
        for tag, f in (("007", 0.07), ("008", 0.08)):
            out.append({"tag": tag, "f": f, "scale": 1.0 - f, "seed": seed,
                        "prefix": "R287BF%s_s%d" % (tag, seed)})
...
def run_cell(i, c):
    ...
    io.open(os.path.join(PAPER, "RUN_BANDFILL_r287_cell%02d.json" % i), "w",
...
def main():
    ...
    for i, c in enumerate(cells()):
        f = os.path.join(PAPER, "RUN_BANDFILL_r287_cell%02d.json" % i)
        if os.path.exists(f):
            continue
        r = run_cell(i, c)
        if r.get("status") == "YIELDED":
            print("stopping: yielded to the user's own work", flush=True)
            write_terminal("r287", sum(1 for _ in glob.glob(os.path.join(PAPER, "RUN_BANDFILL_r287_cell*.json"))), 45, "YIELDED to another sconecmd")
            return
        print("cell %02d %-16s f=%.2f s%-4d wall %6.1f s  t_end %8s  in_band %s"
              % (i, r["prefix"], c["f"], c["seed"], r.get("wall_clock_s", 0),
                 ("%.3f" % r["t_end_s"]) if r.get("t_end_s") is not None else "-",
                 r.get("in_band")), flush=True)
        ...
        if n >= TARGET_IN_BAND:
            ...
            write_terminal("r287", sum(1 for _ in glob.glob(os.path.join(PAPER, "RUN_BANDFILL_r287_cell*.json"))), 45, "REGISTERED STOP")
            return
