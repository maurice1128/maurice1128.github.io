"""run_bandfill_r285.py -- round 285. Launcher for PREREG_bandfill_r285.md (c27a7265...).

Band-fill: max_isometric_force scaled on soleus_l AND gastroc_l, left, two doses
(f = 0.07, 0.08), six seeds each = 12 cells. Registered BEFORE execution.
...
STAGE = os.path.join(ROOT, "bandfill_r285")
PAPER = r"C:\Users\maurice\Desktop\spasticity_paper\paper"

FMAX = {"soleus_l": 3549.0, "gastroc_l": 2241.0}
DOSES = [("f007", 0.07), ("f008", 0.08)]
...
def run_cell(i):
    ...
    io.open(os.path.join(PAPER, "RUN_BANDFILL_r285_cell%02d.json" % i), "w",
...
def main():
    ...
    if a.rest:
        for i in range(len(cells())):
            f = os.path.join(PAPER, "RUN_BANDFILL_r285_cell%02d.json" % i)
            if os.path.exists(f):
                continue
            r = run_cell(i)
