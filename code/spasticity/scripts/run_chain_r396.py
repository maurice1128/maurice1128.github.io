...
STAGE = os.path.join(ROOT, "chain_r396")
...
SETTLE = 1.00
MIN_CYC = 5
SEEDS = [101, 102, 105, 106]        # exactly the seeds that passed Gate G at KV 0.100
...
SP_RUNGS = [("100", 0.100), ("110", 0.110), ("120", 0.120), ("130", 0.130),
            ("140", 0.140)]
WK_RUNGS = [("080", 0.80)]          # root only: this arm climbs the spastic axis alone
SP_ROOT = "R393SPg100"              # the four r393 survivors at KV 0.100
WK_ROOT = "R151W"
    ...
    for k in range(1, len(SP_RUNGS)):
        ...
        for s in SEEDS:
            out.append({
                ...
                "prefix": "R396SPg%s_s%d" % (tag, s),
                "parent_prefix": ("%s_s%d" % (SP_ROOT, s) if k == 1
                                  else "R396SPg%s_s%d" % (ptag, s)),
                "parent_is_root": k == 1})
    for k in range(1, len(WK_RUNGS)):
        ...
        for s in SEEDS:
            out.append({
                ...
                "prefix": "R396WK%s_s%d" % (tag, s),
                "parent_prefix": ("%s_s%d" % (WK_ROOT, s) if k == 1
                                  else "R396WK%s_s%d" % (ptag, s)),
                "parent_is_root": k == 1})
    return out
...
def stage():
    ...
    p = os.path.join(PAPER, "CHAIN_STAGE_r396.json")
...
# ------------------------------------------------------------------ running -----------------
def cell_json(i):
    return os.path.join(PAPER, "RUN_CHAIN_r396_cell%02d.json" % i)
...
def aggregate():
    ...
    out["survival"] = surv
    p = os.path.join(PAPER, "CHAIN_RESULT_r396.json")
...
def rest():
    ...
    for i, c in enumerate(cs):
        ...
        if r.get("status") == "YIELDED":
            write_terminal(len(glob.glob(os.path.join(PAPER, "RUN_CHAIN_r396_cell*.json"))),
...
def headline():
    ...
    p = os.path.join(PAPER, "CHAIN_HEADLINE_r396.json")
    wopen(p).write(json.dumps(out, indent=1) + "\n")
