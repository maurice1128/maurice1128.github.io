...
Leave-one-SEED-out CV, 6 spastic vs 18 weak, all 16 channels, window [1.00, 9.73].
...
ARMS = [("S", "R151S", 1), ("W870", "R174W870", 0), ("W892", "R174W892", 0), ("W915", "R174W915", 0)]
...
assert X.shape == (24, 16) and y.sum() == 6
...
print("accuracy   %d/24 = %.4f   (majority baseline 18/24 = 0.7500)" % (tp + tn, acc))
print("channel picked per fold:", picks)
print("hip_flexion_LmR chosen %d of 6 folds" % hip_picks)

# ---- guard 3: permutation null over the WHOLE procedure, exhaustive
print("\nexhaustive permutation null over C(24,6) = %d assignments ..." % 134596)
ge = tot = 0
for comb in itertools.combinations(range(24), 6):
    lab = np.zeros(24, int); lab[list(comb)] = 1
    r = run_cv(lab)
    if r is None:
        continue
    p2, t2, _ = r
    tot += 1
    if (p2 == t2).sum() / 24.0 >= acc:
        ge += 1
print("assignments evaluated %d   accuracy >= observed in %d   proportion %.6f" % (tot, ge, ge / tot))

json.dump({"prereg": "PREREG_classifier_r222.md sha256 c0a13d6758a9e2876230",
           "design": "leave-one-SEED-out, 6 spastic vs 18 weak, 16 channels, window [1.00, 9.73]",
           "confusion": {"TP": tp, "FN": fn, "FP": fp, "TN": tn},
           "accuracy": acc, "majority_baseline": 18 / 24.0,
           "channel_per_fold": picks, "hip_chosen_of_6": hip_picks,
           "permutation_null": {"method": "exhaustive over C(24,6)",
                                "assignments_evaluated": tot, "count_ge_observed": ge,
                                "proportion": ge / tot}},
          io.open(os.path.join(PAPER, "CLASSIFIER_r222.json"), "w", encoding="utf-8"), indent=1)
print("\ndeposited paper/CLASSIFIER_r222.json")
