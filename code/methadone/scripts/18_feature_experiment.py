# -*- coding: utf-8 -*-
"""
Step 18 - FUTURE-WORK experiment: does adding richer features raise the AUC?
Built from RAW data (dated) so patient-history / co-use / time-in-treatment can
be constructed. Features are added in blocks; patient-grouped 5-fold AUC is
reported after each block (gradient-boosted trees).

Blocks:
  A baseline : dose-trajectory (±28 normalized dose at key offsets + 4 slopes) + attendance
  B +absolute dose at test
  C +amphetamine co-use (±30 d)
  D +time in treatment (days since patient's first dose record)
  E +urine history (prior morphine tests, prior positive rate, last result, days since last)  <-- big lever
"""
import os as _os
...
# --- urine (both morphine & amphetamine) ---
...
for r in it:
    if r[2] in (0,1) and r[3] in (0,1):
        d=roc2d(r[1])
        if d is not None: ur.append((int(r[0]),d,int(r[2]),int(r[3])))  # pid,date,drug(0=amph,1=morph),pos
wb.close()
ur=pd.DataFrame(ur,columns=['pid','date','drug','pos'])
mor=ur[ur.drug==1].drop_duplicates(['pid','date']).sort_values(['pid','date']).reset_index(drop=True)
amp=ur[ur.drug==0]
print('morphine tests:',len(mor),' amph tests:',len(amp))
...
amp_by={p:g['date'].values for p,g in amp[amp.pos==1].groupby('pid')}  # amph-positive dates
...
for t in mor.itertuples(index=False):
    ...
    # normalized dose at offsets (ffill via nearest available)
    normvals=[]
    for o in offs:
    ...
    # urine history (prior morphine tests)
    ...
    rows.append(dict(pid=t.pid,pos=t.pos,d0=normvals[4],
        **{f'nd{o}':v for o,v in zip(offs,normvals)},pmn=pmn,pmx=pmx,amn=amn,amx=amx,att=att,
        absd=absd,amp_co=amp_co,tit=tit,n_prior=n_prior,prior_posrate=prior_posrate,last_res=last_res,days_since=days_since))
df=pd.DataFrame(rows).fillna(1.0)
print('analysis tests:',len(df),' pos rate:',round(df.pos.mean(),3),' patients:',df.pid.nunique())

base=[f'nd{o}' for o in offs]+['pmn','pmx','amn','amx','att']
blocks=[('A baseline (dose traj + attendance)',base),
        ('B +absolute dose',base+['absd']),
        ('C +amphetamine co-use',base+['absd','amp_co']),
        ('D +time in treatment',base+['absd','amp_co','tit']),
        ('E +urine history (big lever)',base+['absd','amp_co','tit','n_prior','prior_posrate','last_res','days_since'])]
y=df.pos.values; groups=df.pid.values
print('\n=== AUC (GroupKFold-5 by patient, gradient boosting) ===')
prev=None
for name,cols in blocks:
    X=df[cols].values; aucs=[]
    for tr,te in GroupKFold(5).split(X,y,groups):
        m=HistGradientBoostingClassifier(random_state=0).fit(X[tr],y[tr]); aucs.append(roc_auc_score(y[te],m.predict_proba(X[te])[:,1]))
    a=np.mean(aucs); delta=f' ({a-prev:+.3f})' if prev is not None else ''
    print(f'  {name:38s}: AUC = {a:.3f}{delta}'); prev=a
