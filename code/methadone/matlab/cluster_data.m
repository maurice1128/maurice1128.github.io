...
%% PCA
X = [All_raw_norm{1}(location1(location),:),All_raw_norm{2}(location1(location),:)]
...
scatter(score(:,1),score(:,2),'.')
hist3([score(:,1),score(:,2)],'CDataMode','auto','FaceColor','interp')
scatter(X(:,1),X(:,2),'.')
...
scatter(theta,rho)
X = [theta,rho]
h = histogram(theta,'Normalization','probability')
...
X = [slope_burst_min_7da{1}(location1(location));slope_burst_max_7db{1}(location1(location))]'

h = histogram(freq_All{1}(location1(location)),'NumBins',40)
scatter(X(:,1),X(:,2))
hist3([X(:,1),X(:,2)])
hist3([X(:,1),X(:,2)],'CDataMode','auto','FaceColor','interp')
X = X(location)
...
% hist3(X,'CDataMode','auto','FaceColor','interp','Nbins',[100,100])
 idx = dbscan(X,eps,minpts); % The default distance metric is Euclidean distance
...
rng(5); % Reset seed for common start values
GMModel = fitgmdist(X,10,'RegularizationValue',0.1)


figure
y = [zeros(1701,1);ones(1700,1)];
...
X = [All_raw_norm{1}(a,22:36),All_raw_norm{2}(a,22:36)]
[coeff,score,latent,tsquared,explained] = pca(X);
