...
temp  = All_fill_norm{3}(real_negative,22:36);
temp_s = diff(temp');
temp_s = sum(abs(temp_s))
location = find(temp_s==0)';
location_check = setdiff([1:683],location)
data =  [All_fill_norm{3}(real_negative(location),:),All_fill_norm{4}(real_negative(location),:)]
x1 = data-mu;
...
rat_1 = length(pca_1)/length(real_negative)
rat_2 =length(pca_2)/length(real_negative)
rat_3 =length(pca_3)/length(real_negative)
rat_4 =length(pca_4)/length(real_negative)
rat_5 =length(pca_5)/length(real_negative)
...
thetamin = theta;
X_rn = [slope_burst_min_7da{3}(real_negative(location_check));slope_burst_min_7db{3}(real_negative(location_check));dosage_All{3}(real_negative(location_check))]'
...
X_rn = [slope_burst_max_7da{3}(real_negative(location_check));slope_burst_max_7db{3}(real_negative(location_check))]'
...
aim_option = [length(pca_1)/(length(real_negative));length(pca_2)/(length(real_negative));length(pca_3)/(length(real_negative));length(pca_4)/(length(real_negative));length(pca_5)/(length(real_negative));length(xy{1,1}')/(length(real_negative));length(xy{1,3}')/(length(real_negative));length(xy{1,5}')/(length(real_negative));length(xy{3,1}')/(length(real_negative));length(xy{3,3}')/(length(real_negative));length(xy{3,5}')/(length(real_negative));length(xy{5,1}')/(length(real_negative));length(xy{5,3}')/(length(real_negative));length(xy{5,5}')/(length(real_negative));length(xy{7,1}')/(length(real_negative));length(xy{7,3}')/(length(real_negative));length(xy{7,5}')/(length(real_negative))]
...
title([name{special} ' Percentage : '  num2str(100*length(aim)/683) '%'])
saveas(gcf,['/Users/mauricewang/Desktop/mesodon_code/Picture/cluster_28_7_ab_ver1_negative/' name{special} ' '  num2str(100*length(aim)/683) '%_s.png'])
close all
num = num+100*length(aim)/683
end
